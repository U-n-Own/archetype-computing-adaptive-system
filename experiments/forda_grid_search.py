#!/usr/bin/env python3
"""
Grid search for FordA dataset hyperparameters
...
FIXED_PARAMS = {
    ...
    'dataroot': 'data/UCR/FordA',  # Corrected path
    'cpu': True,
    'use_test': True,
}

# Results directory
RESULTS_DIR = Path('experiments/forda_grid_search_results')
...
def run_experiment(model_type, params, antisymmetric=False, coupling_epsilon=None):
    ...
    """
    # Build command
    cmd = ['python3', 'experiments/forda.py']
    ...
    if model_type == 'ron':
        ...
        log_filename = f"FordA_log_RON_{topology}{ac_suffix}.txt"
    elif model_type == 'esn':
        log_filename = f"FordA_log_ESN{ac_suffix}.txt"
    else:
        log_filename = f"FordA_log_{model_type.upper()}{ac_suffix}.txt"
...
╔══════════════════════════════════════════════════════════════════════════════╗
║                                                                              ║
║                   FordA Hyperparameter Grid Search                           ║
║                                                                              ║
...
║                                                                              ║
║  IMPORTANT: Update FIXED_PARAMS['dataroot'] with your FordA dataset path!   ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
    """)
    
    # Check if dataroot needs to be updated
    if FIXED_PARAMS['dataroot'] == '/path/to/FordA':
        print("⚠️  WARNING: Please update the dataroot path in the script!")
        print("   Edit FIXED_PARAMS['dataroot'] at the top of this file.")
