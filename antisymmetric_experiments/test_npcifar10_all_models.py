...
ESN_RHO_VALUES = [900]  # List of rho values to test for 5-layer ESN, e.g., [0.7, 0.8, 0.9]
ESN_INPUT_SCALING = 1  # Input scaling for ESN
ESN_LEAKY = 0.001  # Leaky rate for ESN

# RON Hyperparameters
RON_DT = 0.034  # Time step for RON
...
RON_EPSILON_CENTER = 12.7  # Center value for epsilon range
RON_EPSILON_RANGE = 0.5  # Range for epsilon
RON_GAMMA_CENTER = 1.3  # Center value for gamma range
RON_GAMMA_RANGE = 0.5  # Range for gamma
...
def train_and_evaluate(model_name, model, train_loader, valid_loader, test_loader, device, seq_length):
    ...
    valid_acc = evaluate_model(model, valid_loader, classifier, scaler, device, seq_length)
    test_acc = evaluate_model(model, test_loader, classifier, scaler, device, seq_length)
    ...
    if args.model_type in ["esn", "both"]:
        ...
        for trial in range(N_TRIALS):
            ...
            result = train_and_evaluate(
                ...
                model, train_loader, valid_loader, test_loader, device, SEQ_LENGTH
            )
