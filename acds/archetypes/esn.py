...
class ReservoirCell(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.cycle:
            ...
            if self.units == 1:
                ...
                projection_kernel = sparse_recurrent_tensor_init(self.units, C=cycle_connectivity)
                projection_kernel = spectral_norm_scaling(projection_kernel, spectral_radius)
                self.projection_kernel = nn.Parameter(projection_kernel, requires_grad=False)
        else:
            # No cycle mode 
            self.kernel = (
    ...
    def forward(self, xt: torch.Tensor, h_prev: torch.Tensor, first_layer: bool = False, h_last: Optional[torch.Tensor] = None, h_prev_layer: Optional[torch.Tensor] = None, h_next_layer: Optional[torch.Tensor] = None):
        ...
        if self.antisymmetric:
            ...
            # add diffusion term -I*\gamma to the W_rec
            # This corresponds to subtracting gamma * h_prev from the total input
            total_input = total_input - self.gamma * h_prev.to(dtype=xt.dtype)
