...
# Attempt to import ADIAC getter if available in the environment
try:
    from acds.benchmarks.adiac.getter import get_adiac_data
except ImportError:
    get_adiac_data = None
...
# -------------------------------------------------------------
# Generators for new datasets (NARMA, Lorenz96)
# -------------------------------------------------------------
def generate_narma(length: int, order: int, seed: int = 42) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Generate NARMA time series based on Eq. (17) in the paper.
    y(t) = 0.3y(t-1) + 0.01y(t-1)sum(y(t-i)) + 1.5x(t-d)x(t-1) + 0.1
    """
    rng = np.random.default_rng(seed)
    # Input x(t) uniformly distributed in [0, 0.5]
    x = rng.uniform(0, 0.5, size=length)
    y = np.zeros(length)
    
    # Initialize first few steps to 0 or small random values to avoid index errors
    # The paper doesn't specify init, assuming 0 for history < 0
    for t in range(order, length):
        sum_y = np.sum(y[t-order:t])
        term1 = 0.3 * y[t-1]
        term2 = 0.01 * y[t-1] * sum_y
        term3 = 1.5 * x[t-order] * x[t-1]
        y[t] = term1 + term2 + term3 + 0.1

    # Task: Input is x(t), Target is predicting y(t)
    # Note: Some NARMA tasks treat it as autonomous prediction, but Eq 17 relies on input x.
    # Standard RC NARMA task: Input = x, Target = y
    return torch.tensor(x, dtype=torch.float32), torch.tensor(y, dtype=torch.float32)


def generate_lorenz96(length: int, lookahead: int, F: float = 8.0, N: int = 5, dt: float = 0.01, seed: int = 42) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Generate Lorenz96 time series based on Eq. (15) in the paper.
    ...
    # Warmup to settle on attractor (arbitrary small amount not specified, doing 100 steps)
    for _ in range(100):
        dxdt = np.zeros(N)
        for i in range(N):
    ...
    for _ in range(length + lookahead):
        dxdt = np.zeros(N)
        for i in range(N):
            dxdt[i] = (state[(i + 1) % N] - state[(i - 2) % N]) * state[(i - 1) % N] - state[i] + F
        state = state + dxdt * dt
    ...
    # Task: Predict future state. Input = x(t), Target = x(t + lookahead)
    inputs = history[:length]
    targets = history[lookahead : length + lookahead]
...
@torch.no_grad()
def evaluate_forecasting(model: torch.nn.Module, dataset: torch.Tensor, target: torch.Tensor, clf, scaler, washout: int) -> float:
    # Generalized from evaluate_mackey_glass to support multivariate inputs (Lorenz96)
    input_dim = dataset.shape[-1] if dataset.dim() > 1 else 1
    seq = dataset.reshape(1, -1, input_dim).to(next(model.parameters()).device)
    ...
    # Target shape adjustment - must also apply washout to targets
    target_dim = target.shape[-1] if target.dim() > 1 else 1
    target_np = target[washout:].reshape(-1, target_dim).numpy()
...
def get_data_config(dataset_name: str, dataroot: str, batch_size: int, seed: int, device: torch.device, mg_lag: int, mg_washout: int):
    ...
    if dataset_name in {"forda", "fordb", "oliveoil", "adiac"}:
        loader_map = {
            "forda": get_forda_data,
            "fordb": get_fordb_data,
            "oliveoil": get_olive_oil_data,
            "adiac": get_adiac_data
        }
        
        if dataset_name == "adiac" and get_adiac_data is None:
             raise ImportError("ADIAC getter not found. Ensure 'acds.benchmarks.adiac.getter' is importable.")
        ...
        loaders = loader_map[dataset_name](
            ...
            whole_train=False,
            for_rc=True,
            # ADIAC uses 95-5 split for sMNIST but 70-30 or standard for UCR. 
            # ADIAC is standard UCR, using valid_ratio default or implied by getter.
            # Assuming standard getter signature (valid_ratio not explicitly in ADIAC getter, implied or handled inside)
        )
    ...
    # Regression / Forecasting Datasets
    ...
    if dataset_name.startswith("narma"):
        # NARMA: 10000 steps total. 5000 train, 2500 val, 2500 test.
        order = int(dataset_name.replace("narma", ""))
        inputs, targets = generate_narma(length=10000, order=order, seed=seed)
        
        # Splits
       
        train_in, train_tg = inputs[:5000], targets[:5000]
        val_in, val_tg = inputs[5000:7500], targets[5000:7500]
        test_in, test_tg = inputs[7500:], targets[7500:]
        
        return {
            "task": "regression",
            "train": (train_in, train_tg),
            "valid": (val_in, val_tg),
            "test": (test_in, test_tg),
            "input_size": 1,
            "washout": 200, # Per paper
        }
        
    if dataset_name.startswith("lz"):
        # Lorenz96: 1200 steps total. 400 train, 400 val, 400 test.
        lookahead = int(dataset_name.replace("lz", "")) # e.g. 25 or 50
        inputs, targets = generate_lorenz96(length=1200, lookahead=lookahead, seed=seed)
        
        train_in, train_tg = inputs[:400], targets[:400]
        val_in, val_tg = inputs[400:800], targets[400:800]
        test_in, test_tg = inputs[800:], targets[800:]
        ...
        return {
            ...
            "valid": (val_in, val_tg),
            "test": (test_in, test_tg),
            "input_size": 5, # Lorenz96 is 5D in this paper
            "washout": 200, # Per paper
        }
...
def objective(trial: optuna.Trial, arch: str, dataset_name: str, model_type: str, base_dataroot: str, mg_lag: int, mg_washout: int) -> float:
    ...
    # Load config to determine input size before model init
    # NOTE: We need input_size to define n_hid correctly if we want fixed params, 
    # but get_data_config needs batch info. We do a lightweight check for input_size.
    if dataset_name.startswith("lz"):
        input_size = 5
    else:
        input_size = 1
    ...
    # Ensure input_size matches actual data config
    ...
    if config["model"] == "esn":
        ...
        model = DeepRandomizedOscillatorsNetwork(
            ...
            rho=config["rho"],
            input_scaling=config["inp_scaling"],
            inter_scaling=config.get("inter_scaling", 1),
            device=device,
            antisymmetric_coupling=antisymmetric_flag,
            coupling_epsilon=config["coupling_epsilon"],
            cycle=cycle_flag,
    ...
    if data_config["task"] == "classification":
        ...
        # Regression / Forecasting (MackeyGlass, NARMA, Lorenz)
        train_dataset, train_target = data_config["train"]
        valid_dataset, valid_target = data_config["valid"]
        
        # Prepare inputs for model: [Batch=1, Seq, Dim]
        input_dim = data_config["input_size"]
        train_seq = train_dataset.reshape(1, -1, input_dim).to(device)
        
        # Get reservoir features
        ...
        # Targets: [Seq-Washout, OutDim] - must also apply washout to targets
        washout = data_config["washout"]
        target_dim = train_target.shape[-1] if train_target.dim() > 1 else 1
        y = train_target[washout:].reshape(-1, target_dim).numpy()
        
        clf = Ridge(max_iter=1000).fit(X, y)

        valid_score = evaluate_forecasting(
            model,
            valid_dataset,
            valid_target,
            clf,
...
if __name__ == "__main__":
    ...
    try:
        ...
        dataset_list = (
            ...
            if datasets_env
            else ["forda", "fordb", "oliveoil", "mackeyglass", "adiac", "narma30", "narma60", "lz25", "lz50"]
        )
        architectures = (
        ...
        for dataset in dataset_list:
            # Direction: minimize error for regression, maximize accuracy for classification
            if dataset in {"mackeyglass"} or dataset.startswith("narma") or dataset.startswith("lz"):
                direction = "minimize"
            else:
                direction = "maximize"
