#!/usr/bin/env python3
"""
Compare Antisymmetric vs Cycle (Cyclic Standard) ESN connections
...
def connection_mode_search(train_loader, valid_loader, device, mode='antisymmetric', 
    ...
    """
    mode_name = "ANTISYMMETRIC" if mode == 'antisymmetric' else "CYCLE (Cyclic Standard)"
    print(f"\n{'='*70}")
    ...
    if mode == 'antisymmetric':
        ...
        base_grid['epsilon'] = [0.0]  # No epsilon for cycle mode
...
def plot_comparison(antisymmetric_results, cycle_results, n_layers, save_dir="./plots/antisym_vs_cycle"):
    ...
    # Overall title
    plt.suptitle(f'Antisymmetric vs Cycle Mode Comparison ({n_layers} layers)', 
    ...
    stats = {
        'Mean Accuracy': [np.mean(anti_accs), np.mean(cycle_accs)],
        'Best Accuracy': [max(anti_accs), max(cycle_accs)],
        'Std Dev': [np.std(anti_accs), np.std(cycle_accs)],
        'Mean Time (s)': [np.mean(anti_times), np.mean(cycle_times)],
    }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Compare Antisymmetric vs Cycle ESN')
    ...
    print(f"\n{'='*70}")
    print("ANTISYMMETRIC vs CYCLE MODE COMPARISON")
    print(f"{'='*70}")
    ...
    # Save results
    save_dir = "./plots/antisym_vs_cycle"
    os.makedirs(save_dir, exist_ok=True)
